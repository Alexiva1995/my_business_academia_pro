@if(!$presentations->isEmpty())
    <div class="m-1">
        ...
        @else
            @foreach($presentations as $presentation)
                <ul class="list-group">
                    <li style="list-style: none;"><a href="{{route ('download_resource_file', [$event_id, $presentation->id])}}" class="btn btn-primary btn-block" target="_blank">{{$presentation->title}}</a></li>
                </ul>
            @endforeach
        @endif
    </div>
