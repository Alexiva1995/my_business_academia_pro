...
<div class="container-fluid">
  ...
  $(document).ready(function(){
    ...
    function showResponseForm($id){
      $("#response_link_"+$id).css('display', 'none');
      $("#response_div_"+$id).css('display', 'block');
    }
    
    function newResponse($id){
        $("#store_response_submit_"+$id).css('display', 'none');
        $("#store_response_loader_"+$id).css('display', 'block');
        var route = "https://mybusinessacademypro.com/academia/courses/lesson/comments";
        var parametros = $('#store_response_form_'+$id).serialize();
        $.ajax({
            url:route,
            type:'POST',
            data:  parametros,
            success:function(ans){
                $("#response_"+$id).val("");
                $("#store_response_loader_"+$id).css('display', 'none');
                $("#store_response_submit_"+$id).css('display', 'block');
                $("#comments_section").html(ans);
            }
        });
    }
